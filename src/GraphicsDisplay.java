...
public class GraphicsDisplay extends JPanel {
    ...
    public GraphicsDisplay() {
        ...
        graphicsStroke = new BasicStroke(2.0f, BasicStroke.CAP_BUTT,
                BasicStroke.JOIN_ROUND, 10.0f, new float[]{20, 5, 5, 5, 5, 5, 10, 5, 10, 5}, 0.0f);
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        for (Double[] point : graphicsData) {
            ...
            // Проверка условия
            boolean isSquare = isIntegerSquare(point[1]);
            canvas.setColor(isSquare ? Color.GREEN : Color.RED);

            // Размер маркера
            int size = 11; // Общий размер
            int halfSize = size / 2;
            int notch = 3; // Размер засечек

            Line2D.Double horizontalLine = new Line2D.Double(// горизонтальная линия креста
                    center.getX() - halfSize, center.getY(),
                    center.getX() + halfSize, center.getY()
            );

            Line2D.Double verticalLine = new Line2D.Double(//вертикальная линия креста
                    center.getX(), center.getY() - halfSize,
                    center.getX(), center.getY() + halfSize
            );

            Line2D.Double leftNotch = new Line2D.Double(//засечки для горизонтальной линии
                    center.getX() - halfSize, center.getY() - notch,
                    center.getX() - halfSize, center.getY() + notch
            );
            Line2D.Double rightNotch = new Line2D.Double(
                    center.getX() + halfSize, center.getY() - notch,
                    center.getX() + halfSize, center.getY() + notch
            );

            Line2D.Double topNotch = new Line2D.Double(//щасечки для вертикальной линии
                    center.getX() - notch, center.getY() - halfSize,
                    center.getX() + notch, center.getY() - halfSize
            );
            Line2D.Double bottomNotch = new Line2D.Double(
                    center.getX() - notch, center.getY() + halfSize,
                    center.getX() + notch, center.getY() + halfSize
            );
            ...
            canvas.draw(leftNotch);
            canvas.draw(rightNotch);
            canvas.draw(topNotch);
            canvas.draw(bottomNotch);
        }
    }
    // Метод для проверки, является ли целая часть числа квадратом целого числа
    ...
    protected void paintAxis(Graphics2D canvas) {
        ...
        if (minY <= 0.0 && maxY >= 0.0) {
            ...
            // Вывести надпись в точке с вычисленными координатами
            canvas.drawString("x", (float) (labelPos.getX() - bounds.getWidth() - 10), (float) (labelPos.getY() - bounds.getY() + 10));
        }
    }
